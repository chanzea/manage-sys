...
import {
  getSystemLogListTask,
  getListTaskActionType
...
export default {
  ...
  created () {
    ...
    // this.value = [this.getDate(-7), this.getDate(0)];
    this.getListActionType();
    this.getSystemLogListTask();
  },
  computed: {
    isNext () {
  ...
  methods: {
    ...
    getSystemLogListTask() {
      ...
      getSystemLogListTask(params).then(res => {
        const {taskActionLogList,userList,count,taskList} = res;
        this.userList = userList;
        this.taskList = taskList;
        this.data = taskActionLogList.map(item => {
